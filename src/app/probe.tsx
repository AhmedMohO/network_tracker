...
export default function Probe() {
  ...
  const deviceProbeTimers = useRef<{ interval?: ReturnType<typeof setInterval>; timeout?: ReturnType<typeof setTimeout> }>({});
  const prevDeviceCounters = useRef<{ mobileRx: number; mobileTx: number; totalRx: number; totalTx: number } | null>(null);

  const [appProbeSamples, setAppProbeSamples] = useState<AppProbeSample[]>([]);
  const [appProbeRunning, setAppProbeRunning] = useState(false);
  const appProbeTimers = useRef<{ interval?: ReturnType<typeof setInterval>; timeout?: ReturnType<typeof setTimeout> }>({});
  ...
  const runDeviceProbe = () => {
    ...
    deviceProbeTimers.current.interval = setInterval(() => {
      ...
      const anyNegative = [c.mobileRx, c.mobileTx, c.totalRx, c.totalTx, prev.mobileRx, prev.mobileTx, prev.totalRx, prev.totalTx].some(
        (v) => v < 0
      );
      const toMbps = (now: number, before: number) => (now - before) / 1_000_000; // 1s interval
      setDeviceSamples((s) => [
        ...s,
        {
  ...
  const runAppLiveProbe = () => {
    ...
    appProbeTimers.current.interval = setInterval(async () => {
      ...
      try {
        const result = await NetworkUsage.getAppUsage({ start, end, network: "MOBILE" });
        ...
        setAppProbeSamples((s) => [
          ...s,
          { time: new Date().toISOString(), rowCount: -1, changed: [String(e)] },
        ]);
      }
    }, 2000);
  ...
  return (
    <ScrollView contentContainerStyle={{ padding: 16, gap: 12 }}>
      ...
      <Text style={{ fontSize: 20, fontWeight: "600" }}>Device live speed probe</Text>
      <Text>Samples getDeviceCounters() every 1s for 30s; renders per-second delta as MB/s.</Text>
      <Button
        title={deviceProbeRunning ? "Running (30s)..." : "Run device live speed probe"}
      ...
      <Text style={{ fontSize: 20, fontWeight: "600" }}>Per-app live feasibility probe</Text>
      <Text>
        Every 2s for 60s, queries getAppUsage for the trailing 10s window (MOBILE) and logs which
        apps' byte totals changed since the previous sample.
      </Text>
      <Button
        title={appProbeRunning ? "Running (60s)..." : "Run per-app live feasibility probe"}
